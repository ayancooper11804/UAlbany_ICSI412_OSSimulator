public class TestMemory1 extends UserlandProcess {
    ...
    void main() throws InterruptedException {
        ...
        while (true) {
            ...
            if (address1 != -1) {
                System.out.println("TestMemory1 first allocation at address: " + address1);

                // Write a byte value of 50 to the first allocated address
                byte valueToWrite1 = 50;
                Hardware.Write(address1, valueToWrite1);
                System.out.println("TestMemory1 Written byte value: " + valueToWrite1 + " at address: " + address1);

                // Read the byte value from the first allocated address
                byte readValue1 = Hardware.Read(address1);
                System.out.println("TestMemory1 Read byte value: " + readValue1 + " from address: " + address1);

            }
            else {
                // Log if the first allocation was unsuccessful
                System.out.println("TestMemory1 first allocation failed");
            }
            ...
            if (address2 != -1) {
                System.out.println("TestMemory1 second allocation at address: " + address2);

                // Write a byte value of 100 to the second allocated address
                byte valueToWrite2 = 100;
                Hardware.Write(address2, valueToWrite2);
                System.out.println("TestMemory1 Written byte value: " + valueToWrite2 + " at address: " + address2);

                // Read the byte value from the second allocated address
                byte readValue2 = Hardware.Read(address2);
                System.out.println("TestMemory1 Read byte value: " + readValue2 + " from address: " + address2);
            }
            else {
                // Log if the second allocation was unsuccessful
                System.out.println("TestMemory1 second allocation failed");
            }
            ...
            if (address1 != -1 && OS.FreeMemory(address1, requestedSize)) {
                ...
                System.out.println("TestMemory1 freed first allocation");
            }
            if (address2 != -1 && OS.FreeMemory(address2, nextRequestedSize)) {
                // Confirm second memory was freed
                System.out.println("TestMemory1 freed second allocation");
            }
